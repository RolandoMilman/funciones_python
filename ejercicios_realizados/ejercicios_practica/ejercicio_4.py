...
def lista_aleatoria(inicio, fin, cantidad):
    ...
    for numero in range(cantidad):
        ...
        while True:
            # return numero
            # Verifico que no exista en la lista, en ese caso lo agrego a  la lista 
            numero_aleatorio = random.randrange(inicio, fin+1)
            cuenta = lista.count(numero_aleatorio)
            if (cuenta>=1):
                print('El número ', numero_aleatorio, 'se repite')
            else:
...
if __name__ == '__main__':
    ...
    # 1 y 6
    print("Lista aleatorio generada", lista_aleatoria(inicio, fin, cantidad))
